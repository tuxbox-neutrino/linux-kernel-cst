...
static int stb_gpio_int_set_type(struct irq_data *data, unsigned trigger)
{
...
static void handle_stb_gpio_irq(unsigned irq, struct irq_desc *desc)
{
//SRAO	struct stb_gpio_chip * ach = irq_get_irq_data(irq);
...
int gpio_interrupt_set_type(unsigned piono, unsigned trigger)
{
	...
	if (trigger & IRQ_TYPE_LEVEL_HIGH)
        {
           val = 0;
           irq_set_chip_and_handler(irq, &stb_gpio_irq_chip, handle_level_irq);
	}
...
static int stb_gpio_get(struct gpio_chip *chip,
						unsigned offset)
{
	struct stb_gpio_chip *ach = to_ach(chip);
...
static int stb_gpio_to_irq(struct gpio_chip *chip,
						unsigned offset)
{
	struct stb_gpio_chip *ach = to_ach(chip);
...
static int  gpio_stb_probe(struct platform_device *pdev)
{
	...
	match = of_match_device(gpio_stb_dt_match, &pdev->dev);
