...
static int kronos_platform_notifier(struct notifier_block *nb,
...
static struct notifier_block kronos_platform_nb = {
	.notifier_call = kronos_platform_notifier,
};

static void __init kronos_dt_init(void)
{
	int ret;
#ifdef CONFIG_ARCH_HAS_BARRIERS
	...
	if(pDummyUncachedWrite == NULL) {
		pr_err("Failed to allocate DMA coherent memory for dummy write/read\n");
		BUG();
	}
#endif
	bus_register_notifier(&platform_bus_type, &kronos_platform_nb);
...
static const char * const kronos_dt_board_compat[] = {
	"entr,kronos",
	NULL,
};

DT_MACHINE_START(KRONOS_DT, "Entropic Kronos STB Platform with FDT")
	.smp		= smp_ops(kronos_smp_ops),
	.map_io		= kronos_map_io,
	.init_irq	= irqchip_init,
	.init_time	= kronos_timer_init,
	.init_machine	= kronos_dt_init,
	.restart	= kronos_restart,
	.dt_compat	= kronos_dt_board_compat,
MACHINE_END
