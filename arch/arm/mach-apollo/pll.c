...
#define HM_CGU_BASE                    (SOC_MMIO_CLOCK_BASE)
 
#define CGU_PLL_ARM_CON1_CTL_REG       (HM_CGU_BASE + 0x60)
 
#define CGU_PLL_ARM_CON2_CTL_REG       (HM_CGU_BASE + 0x64)
/* PLL DIRECTO */
...
unsigned long get_arm_freq(void)
{
   ...
   return (u32)ullFreq;
#else /* FIXME */
	unsigned long pll_arm_con1, freq;

	pll_arm_con1 = readl(CGU_PLL_ARM_CON1_CTL_REG);
	...
	pr_info("Cortex A9 frequency at %lu MHz\n", (freq/1000)/1000);
	return freq;
#endif
}
